...
const AddWeekModal = ({ isOpen, onClose, onWeekAdded, courseId, existingWeeks }) => {
  ...
  const validateForm = () => {
    ...
    if (existingWeeks.some(week => week.weekNumber === weekData.weekNumber)) {
      newErrors.weekNumber = "Week number already exists";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post("/admin/syllabus/add", cleanedData, {
        headers: { Authorization: `Bearer ${token}` }
      });

      onWeekAdded(response.data.week);
      onClose();
      resetForm();
    } catch (error) {
      console.error("Error adding week:", error);
      setErrors({ submit: "Failed to add week" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">Add New Week</h2>
          <button
            onClick={onClose}
