...
const AddCourse = () => {
  ...
  const fetchCategories = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:3000/api/v1/admin/coursecategory', {
  ...
  const fetchMentors = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:3000/api/v1/admin/mentors', {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            <Link to="/admin/courses">
              <Button variant="white" className="flex items-center space-x-2 border border-gray-300">
                <ArrowLeft size={20} />
                <span>Back to Courses</span>
              </Button>
            </Link>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Title, Category, Duration, Mentors, Start Date, End Date */}
            {/* Description */}
            {/* Image Upload */}
            {/* Submit Buttons */}
            {/* ... same JSX as your original with no type annotations */}
          </form>
        </div>
      </main>
